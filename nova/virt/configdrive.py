...
configdrive_opts = [
    cfg.StrOpt('config_drive_format',
               default=None,
               help='Config drive format. One of iso9660, vfat, ext4, '
                    'or unset to auto-detect'),
...
class ConfigDriveBuilder(object):
    ...
    def make_drive(self, path, image_type="raw"):
        ...
        """
        LOG.debug("CONF.config_drive_format=%s", CONF.config_drive_format)
        fs_format = CONF.config_drive_format
        if fs_format is None:
            if image_type == 'raw':
                fs_format = FS_FORMAT_ISO9660
            elif image_type == 'ploop':
                fs_format = FS_FORMAT_EXT4

        LOG.debug("fs_format=%s image_type=%s", fs_format, image_type)
